...
def mip(facilities, customers, verbose=False, num_threads=None, time_limit=None):
    ...
    m.setObjective(LinExpr((facilities[j].setup_cost, x[j])
                   ...
                   LinExpr((length(customers[i].location, facilities[j].location), y[(i, j)])
    ...
    m.addConstrs((y.sum(i, "*") == 1
                  for i in range(c_count)),
                 name="customer-constraint")

    m.addConstrs((x[j] >= y.sum(i, j)
                  for i in range(c_count)
                  for j in range(f_count)),
                 name="facility-customer-correct")
    ...
    matrix = [[int(m.getVarByName("y[{},{}]".format(i, j)).x) for j in range(f_count)] for i in range(c_count)]
    solution = [j for i in range(c_count) for j in range(f_count) if matrix[i][j] == 1]
